...
class DashboardServerTests(unittest.TestCase):
    def test_index_html_exposes_botinance_shell(self) -> None:
        ...
        self.assertIn("XRP/JPY 模拟盯盘", INDEX_HTML)
        self.assertIn("function fmtSymbol", INDEX_HTML)
        self.assertIn("fmtSymbol(c.symbol, c.quoteAsset)", INDEX_HTML)
        self.assertIn("top-bar", INDEX_HTML)
        self.assertIn("top-name-row", INDEX_HTML)
        self.assertIn("topVersion", INDEX_HTML)
        self.assertIn("ops_version", INDEX_HTML)
        ...
        self.assertIn("\"单据ID\", \"订单状态\", \"方向\", \"数量\", \"挂单/成交价\", \"价差\", \"冻结\", \"手续费\", \"已实现\", \"时间\"", INDEX_HTML)
        ...
        self.assertIn("Boti已实现操作利润线", INDEX_HTML)
        self.assertIn("已实现 ${fmtCurrency", INDEX_HTML)
        ...
        self.assertIn("已加载 ${ledger.length} 条", INDEX_HTML)
        self.assertIn("扫描 ${meta.scan_lines} 行", INDEX_HTML)
        ...
        self.assertIn("keepCompleteTradeRecords", INDEX_HTML)
        self.assertIn("previous.trade_records_complete === true", INDEX_HTML)
        self.assertIn("order_records_refresh_needed", INDEX_HTML)
        self.assertIn("refreshOrderRecords(true)", INDEX_HTML)
        ...
        self.assertIn("策略释放待回补", INDEX_HTML)
        self.assertIn("target_rebuild_buy", INDEX_HTML)
        self.assertIn("目标仓位补仓/建仓", INDEX_HTML)
        self.assertIn("方向允许性 / 最终下单", INDEX_HTML)
        self.assertIn("候选提案", INDEX_HTML)
        self.assertIn("通过提案", INDEX_HTML)
        self.assertIn("外部共识", INDEX_HTML)
        self.assertIn("externalSignalCard", INDEX_HTML)
        self.assertIn("external_consensus", INDEX_HTML)
        self.assertIn("本地/外部", INDEX_HTML)
        self.assertIn("本地 60 / 外部 40 融合明细", INDEX_HTML)
        self.assertIn("外部票选明细", INDEX_HTML)
        ...
        self.assertIn("等待决策轮或关键阈值事件", INDEX_HTML)
        ...
        self.assertIn("executionResult = c.executionResult || {}", INDEX_HTML)
        self.assertIn("operationAction", INDEX_HTML)
        self.assertNotIn("triggerLabel(executionResult.trigger || c.executionReason", INDEX_HTML)
        self.assertIn("模拟限价单已成交", INDEX_HTML)
        self.assertIn("模拟限价单已挂出，尚未成交", INDEX_HTML)
        self.assertIn("未成交已撤单", INDEX_HTML)
        self.assertIn("挂单未成交", INDEX_HTML)
        self.assertIn('"单据ID", "订单状态", "方向", "数量", "挂单/成交价", "价差"', INDEX_HTML)
        self.assertIn('"时间", "单据ID", "状态", "方向", "价格", "数量", "原因"', INDEX_HTML)
        ...
        self.assertIn("大模型决策", INDEX_HTML)
        ...
        self.assertIn("chartMaConfig", INDEX_HTML)
        self.assertIn("movingAverageSeries", INDEX_HTML)
    ...
    def test_build_dashboard_payload_prefers_walk_forward_directory(self) -> None:
        ...
        self.assertAlmostEqual(payload["live_profit_curve"][-1]["realized_pnl"], 0.0)
        ...
        self.assertIn("mtf_trend_interval", payload["runtime_config"])
        self.assertIn("mtf_trend_fast_window", payload["runtime_config"])
