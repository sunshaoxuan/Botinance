# Phase 7 设计文档：高级交易工作台收口

## 目标

在不引入前端构建链、不改交易逻辑、不改 `/api/dashboard` 既有字段语义的前提下，把原报告型 dashboard 收口为本地高级交易工作台。页面默认进入 `实时交易`，用户不需要读取 JSON 即可理解当前行情、持仓、AI 裁决、风险线、回测表现和系统运行状态。
